...
ALTER TABLE robots
ADD COLUMN game_type ENUM('chess', 'doudizhu') NOT NULL DEFAULT 'chess'
AFTER strategy;
...
ALTER TABLE matches
ADD COLUMN robot_third_id INT UNSIGNED NULL
AFTER robot_black_id;

ALTER TABLE matches
ADD COLUMN robot_landlord_id INT UNSIGNED NULL
AFTER robot_third_id;

-- Add foreign keys (ER_DUP_KEYNAME is caught by migrate.ts if already exists)
ALTER TABLE matches
ADD CONSTRAINT fk_matches_robot_third
FOREIGN KEY (robot_third_id) REFERENCES robots(id) ON DELETE SET NULL;

ALTER TABLE matches
ADD CONSTRAINT fk_matches_robot_landlord
